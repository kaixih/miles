...
def iterator():
    return SimpleNamespace(offset=0, micro_batch_size=None, micro_batch_indices=[[0], [1], [2], [3]],
                           rollout_data={'tokens':[[1,2],[3,4],[5,6],[7,8]], 'total_lengths':[2]*4,
...
class ActorProfileTests(unittest.TestCase):
    ...
    def check_hook(self, rank):
        ...
        class Profiler:
            ...
            def export_chrome_trace(self, path):
                Path(path).write_text(json.dumps({'traceEvents':[
                    {'cat':'cpu_op','name':'actor.selected_update'}, {'cat':'kernel','name':'gemm'}]}))
        profiler = SimpleNamespace(profile=Profiler, record_function=lambda name: contextlib.nullcontext(),
                                   ProfilerActivity=SimpleNamespace(CPU=1, CUDA=2))
        torch = SimpleNamespace(distributed=SimpleNamespace(get_world_size=lambda:4, get_rank=lambda:rank, barrier=lambda:None),
            cuda=SimpleNamespace(synchronize=lambda:None), profiler=profiler, __version__='test')
        monkeypatch.setitem(sys.modules, 'torch', torch)
        monkeypatch.setattr(hook.os, 'getuid', lambda:28644)
        monkeypatch.setattr(hook.os, 'getgid', lambda:30)
        monkeypatch.setattr(hook, 'rss_bytes', lambda:100)
        monkeypatch.setattr(hook, 'annotations', lambda *args: contextlib.nullcontext(['test.native_ranges']))
        ...
        data = iterator()
        for step in range(4):
            result = model_module.train_one_step(args,0,step,[data],[],object(),None,1,512,None,0)
            assert result[1] == 2.0
