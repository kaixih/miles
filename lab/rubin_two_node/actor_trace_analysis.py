#!/usr/bin/env python3
"""Analyze one retained rank-0 actor update offline; no runtime operations.
...
def analyze_document(document, device=None):
    ...
    selected = [(i, e) for i, e in events if "user_annotation" in categories(e)
                and e.get("name") == "actor.selected_update"]
    if len(selected) != 1:
        raise ValueError("Require exactly one complete actor.selected_update CPU range")
    ...
    if not kernels:
        raise ValueError("Selected update contains no kernels on the requested device")
    ...
    for label, annotation in SCOPES.items():
        ...
        rows[label] = None if not cpu else {
            ...
            "scope": "same-thread CPU annotation containing the linked operation/API; async GPU execution retained within selected update",
        }
    ...
    return {
        ...
        "window": {"cpu_annotation": "actor.selected_update", "event_index": selected_index,
        ...
        "limitations": [
            ...
            "GPU kernels may execute after their CPU annotation ends; they are clipped only to the selected update window, not the enqueue span.",
            ...
            "Whole-update CUDA synchronization and profiler instrumentation perturb this sample; compare separate unprofiled update timings.",
        ],
    }
...
def analyze(path, expected_sha, receipt_path, device=None):
    ...
    result = analyze_document(json.loads(decoded), device=device)
...
def render_png(result, path):
    ...
    rows = result["timeline_union_intervals_ms"]
    fig, axis = plt.subplots(figsize=(15, max(4, len(rows) * 0.4)))
    for index, (name, spans) in enumerate(rows.items()):
        axis.broken_barh(spans, (index - 0.35, 0.7), facecolors="#3475b9" if name.startswith("CPU") else "#cc792b")
    axis.set_yticks(range(len(rows)), labels=rows)
    axis.invert_yaxis()
    axis.set_xlim(0, result["window"]["duration_ms"])
    axis.set_xlabel("Milliseconds from selected update start; overlapping rows are not additive")
    axis.set_title("Rank 0, rollout 0 / optimizer update 1 — profiled CPU ranges and GPU kernel families")
